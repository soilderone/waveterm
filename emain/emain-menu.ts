...
function makeAppMenuItems(webContents: electron.WebContents): Electron.MenuItemConstructorOptions[] {
    ...
    if (unamePlatform === "darwin") {
        appMenuItems.push(
            ...
            { type: "separator" },
            { role: "hide", label: t("menu.hideApp", { app: electron.app.name }) },
            { role: "hideOthers", label: t("menu.hideOthers") },
            { type: "separator" }
        );
    }
    appMenuItems.push({ role: "quit", label: t("menu.quitApp", { app: electron.app.name }) });
    return appMenuItems;
}
...
function makeViewMenu(
    ...
    return [
        {
            label: isBuilderWindowFocused ? t("menu.reloadWindow") : t("menu.reloadTab"),
        ...
        {
            label: t("menu.relaunchAllWindows"),
            click: () => callbacks.relaunchBrowserWindows(),
        },
        {
            label: t("menu.clearTabCache"),
            click: () => clearTabCache(),
        },
        {
        ...
        {
            label: t("menu.launchOnFullScreen"),
            submenu: [
                {
                    label: t("menu.on"),
                    type: "radio",
...
async function makeFullAppMenu(callbacks: AppMenuCallbacks, workspaceOrBuilderId?: string): Promise<Electron.Menu> {
    ...
    const windowMenu: Electron.MenuItemConstructorOptions[] = [
        { role: "minimize", label: t("menu.minimize"), accelerator: "" },
    ...
    const menuTemplate: Electron.MenuItemConstructorOptions[] = [
        ...
        { role: "fileMenu", label: t("menu.file"), submenu: fileMenu },
        { role: "editMenu", label: t("menu.edit"), submenu: editMenu },
        { role: "viewMenu", label: t("menu.view"), submenu: viewMenu },
    ];
...
function convertMenuDefArrToMenu(
    ...
    for (const menuDef of menuDefArr) {
        ...
        if (menuDef.icon != null) {
            menuItemTemplate.icon = electron.nativeImage.createFromDataURL(menuDef.icon);
        }
