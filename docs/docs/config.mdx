...
| app:uitheme                                                          | string   | UI color theme: `"dark"` (default), `"light"`, or `"system"` to follow the OS setting. Terminal colors are controlled separately by `term:theme`                                                                                                        |
| app:accentcolor                                                      | string   | UI accent color: `"sage"` (default) or `"system"` to follow the macOS accent color chosen in System Settings. The per-view type colors are not affected                                                                                                 |
...
| waveai:hidegettingstarted            | bool     | hide the "Getting Started" card on the empty Wave AI panel (set when the card is dismissed)                                                                                   |
| waveai:defaultaccesslevel            | string   | default Wave AI access level for tabs that have not picked one: "off" (no widget or file access), "readonly" (read widgets and files, no edits), "collab" (default; file reads and edits ask first), or "trust" (reads and edits inside local terminal working directories run without asking; deletes still ask) |
...
```json
{
    ...
    "waveai:defaultaccesslevel": "collab",
