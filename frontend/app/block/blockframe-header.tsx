...
const HeaderEndIcons = React.memo(({ viewModel, nodeModel, blockId }: HeaderEndIconsProps) => {
    ...
    if (showSplitButtons && viewModel?.viewType === "term") {
        const splitHorizontalDecl: IconButtonDecl = {
            elemtype: "iconbutton",
            icon: "columns",
            title: t("term.splitHorizontally"),
            click: (e) => {
                e.stopPropagation();
    ...
    const closeDecl: IconButtonDecl = {
        elemtype: "iconbutton",
        icon: "xmark-large",
        title: `${t("menu.close")}  ${formatKeyDescription("Cmd:w")}`,
...
const BlockFrame_Header = ({
    ...
    return (
        <div
            className={cn("block-frame-default-header", useTermHeader && "!pl-[8px]")}
