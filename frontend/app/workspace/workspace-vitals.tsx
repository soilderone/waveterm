    ...
    subscribe() {
        ...
        const unsubscribe = waveEventSubscribeSingle({
            ...
            handler: (event) => {
                ...
                globalStore.set(this.cpuHistoryAtom, (values) => [
                    ...values.slice(-23),
                    Math.max(0, Math.min(100, sample.values.cpu)),
                ]);
...
export const WorkspaceVitals = memo(() => {
    ...
    return (
        <button
            className="shell-vitals"
            title={t("shell.localMetrics")}
            aria-label={`${t("shell.localMetrics")}: CPU ${cpu != null ? Math.round(cpu) + "%" : "—"}, ${t("view.mem")} ${memory != null ? Math.round(memory) + "%" : "—"}`}
            onClick={() => fireAndForget(() => createBlock({ meta: { view: "sysinfo" } }))}
        >
            <span>CPU</span>
            <svg viewBox="0 0 70 20" width="70" height="20" aria-hidden="true">
                <polyline points={points} fill="none" stroke="currentColor" strokeWidth="1.5" />
            </svg>
            <b>{cpu != null ? `${Math.round(cpu)}%` : "—"}</b>
            <span className="shell-vitals-divider" />
            <span>MEM</span>
            <span className="shell-vitals-meter">
                <i style={{ width: `${memory ?? 0}%` }} />
            </span>
