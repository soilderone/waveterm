...
const AIToolUseBatch = memo(({ parts, isStreaming }: AIToolUseBatchProps) => {
    ...
    return (
        <div className="flex items-start gap-2 p-2 rounded bg-raise/60 border border-border">
            <div className="flex-1">
...
const AIToolUse = memo(({ part, isStreaming }: AIToolUseProps) => {
    ...
    return (
        <div
            className={cn("flex flex-col gap-1 p-2 rounded bg-raise/60 border border-border", statusColor)}
                ...
                {isFileWriteTool &&
                    ...
                    Date.now() - toolData.runts < BackupRetentionDays * 24 * 60 * 60 * 1000 && (
                        <button
                            ...
                            className="flex-shrink-0 px-1.5 py-0.5 border border-border hover:border-borderstrong hover:bg-raise rounded cursor-pointer transition-colors flex items-center gap-1 text-secondary"
...
const AIToolProgress = memo(({ part }: AIToolProgressProps) => {
    ...
    return (
        <div className="flex flex-col gap-1 p-2 rounded bg-raise/60 border border-border">
            <div className="flex items-center gap-2">
