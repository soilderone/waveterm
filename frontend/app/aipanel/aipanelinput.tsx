...
export const AIPanelInput = memo(({ onSubmit, status, model }: AIPanelInputProps) => {
    ...
    return (
        <div className={cn("ai-composer border-t", isFocused ? "border-accent/50" : "border-border")}>
            <input
                ref={fileInputRef}
                type="file"
                multiple
            ...
            <form onSubmit={onSubmit}>
                <div className="relative">
                    <textarea
                        ...
                        className={cn(
                            "w-full  text-primary px-2 py-2 pr-5 focus:outline-none resize-none overflow-auto bg-raise/50"
                        )}
                        style={{ fontSize: "13px" }}
                        rows={2}
                    />
                    <Tooltip content={t("ai.attachFiles")} placement="top" divClassName="absolute bottom-6.5 right-1">
                        <button
                            type="button"
